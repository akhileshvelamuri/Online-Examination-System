...
<body>
	...
	<div class="form-group">
		...
		<label for="subject_id">Subject:</label> <input type="text"
			...
			name="subject_id" value="${requestScope.test.subject_id}" disabled>
		<%-- <c:set var="testsub_id" scope="session" value="${test.subject_id}" />
		<c:set var="quesnsub_id" scope="session"
			value="${question.subject_id}" /> --%>
		<c:forEach var="qn" items="${applicationScope.questionList}">
				<c:if test="${test.subject_id eq qn.subject_id}">
					<label><input type="checkbox" name="question_id" value="${qn.question_id}">${qn.question}</label><br>
				</c:if>
		<input type="hidden" name="test_id" value="${test.test_id}"/>
		
		</c:forEach>
		<button name="action" value="AddQuestionstoTest" class="btn btn-default">Add Questions</button>
